...
public class MenuController {
    ...
    public ResponseEntity<?> getFullRecipeById(@PathVariable int id){
        try {
            MealFullInformationDTO information = service.getFullRecipeById(id);
            GeneralResponse<MealFullInformationDTO> response = new GeneralResponse<>(true, information);
            return new ResponseEntity<>(response, HttpStatus.OK);
        }catch (Exception e){
            GeneralResponse<UpdateResponseDTO> response = new GeneralResponse<>(false, e.getMessage());
            return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
